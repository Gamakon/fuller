...
mod tests {
    ...
    fn named_constants_are_withheld_terminals_appended_after_every_symbol() {
        ...
        never(&now, "init");
        let islands = [Island { lo: 0, hi: 300, elites: 2, tournsize: 20 }, Island { lo: 300, hi: 400, elites: 2, tournsize: 7 }];
        let rates = Rates::with_cleanse(layout, 0.5);
        for generation in 1..=200 {
            now = vary(&now, &islands, &codes, &rates, &GenParams { seed: 5, generation, rnc_lo: -100, rnc_hi: 100, vhead: 0 }).unwrap();
