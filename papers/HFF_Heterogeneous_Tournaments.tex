...
count, with a non-firing rule contributing its \emph{ideal} value $0$ (a missing
pattern means the form lacks that defect, e.g.\ no transcendental to nest --- the
good case, not a failure, so $0$, not $1$). The objective vector is then the same
width for every member, the angle is comparable directly, and at fixed $k$ the
CDF correction is only a monotone reshaping of $\theta$ --- so we rank on the raw
TrueNorth angle and omit it. Sorting the angles totally orders the class; the
minimum is the structurally preferred behaviour-preserving form, no hand-tuned
weights. (The per-form CDF-across-different-$k$ comparison remains valid for
HFF's original setting --- a fixed-objective population --- but not for selecting
among variable-defect members of one e-class.)
